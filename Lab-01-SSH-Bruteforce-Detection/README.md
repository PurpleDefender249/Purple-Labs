...
Kali usually ships with Hydra preinstalled, but let us verify rather than assume.
...
![Hydra installed](media/lab01-01-hydra-version-check.png)
...
![Manual SSH login success](media/lab01-02-manual-ssh-login.png)
...
The Elastic repository should be already configured from Phase 0, so:
...
![Logstash listening on 5514](media/lab01-03-logstash-listening.png)
...
**Notice!:** if you connect via SSH from Kali (rather than the VMware console), `nano` may fail with `Error opening terminal: xterm-256color`. This happens because Metasploitable2's 2008-era terminfo database has no entry for the modern terminal type your SSH session passes through. Fix it for this session:
...
![First event confirmed in Elasticsearch](media/lab01-05-elasticsearch-first-event.png)

### 5.3 Create the Kibana Data View

In your browser (from Kali): `http://192.168.56.102:5601`

1. Hamburger (yes they call it that) menu (☰, top-left) → **Stack Management → Data Views → Create data view**
...
![Kibana Discover showing first event](media/lab01-06-kibana-discover-first-event.png)

**A Big MILESTONE!:** if you see the event in Discover, your full pipeline works. Proceed to the actual attack, and may the force be with you Jedi.
...
![Hydra brute-force attack result](media/lab01-07-hydra-attack-success.png)
...
![Failed login burst in Discover](media/lab01-08-discover-failed-login-burst.png)
...
![SSH failed logins chart](media/lab01-09-lens-failed-logins-chart.png)
...
6. Time window: **1 minute** (You may need to adjust based on the actual time you took to finnish)
...
![Alert rule configuration](media/lab01-10-alert-rule-config.png)
...
![Alert fired in execution history](media/lab01-11-alert-fired-history.png)

---

## Part 9 — Document the Finding

This is the write-up a real analyst would produce. Two separate files in this lab's folder work together:
